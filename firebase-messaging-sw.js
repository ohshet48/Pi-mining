...
const FUNCTIONS_BASE_URL = 'https://us-central1-arcastra-eba62.cloudfunctions.net';
...
async function requestNotificationPermission() {
  try {
    ...
    if (permission === 'granted') {
      ...
      // Get FCM token
      // TODO: VAPID Key'inizi buraya ekleyin (Firebase Console > Cloud Messaging > Web Push certificates)
...
<body>
    ...
    <script>
        ...
        function signup() {
            ...
            const birthDate = document.getElementById('signupBirthDate').value.trim();
            ...
            if (!username || !fullName || !birthDate || !email || !password || !passwordConfirm || !country) {
                showError('Please fill in all required fields');
                return;
            }
            
            // Validate username: 3-12 characters, English only
            ...
            if (!usernameRegex.test(username)) {
                showError('Username: Only English letters, numbers, _ and - allowed');
                return;
            }
            
            // Validate full name: English letters and spaces only
            const fullNameRegex = /^[a-zA-Z\s]+$/;
            if (!fullNameRegex.test(fullName)) {
                showError('Full Name: Only English letters allowed');
                return;
            }
            ...
            // Validate country is from the list
            const isValidCountry = countries.some(c => c.name.toLowerCase() === country.toLowerCase());
            if (!isValidCountry) {
            ...
            if (!match) {
                showError('Birth date must be in DD/MM/YYYY format');
                return;
            }
            ...
            if (year < 1923 || year > 2007) {
                showError('Birth year must be between 1923 and 2007');
                return;
            }
            ...
            if (referralCode) {
                ...
                for (let userEmail in users) {
                    if (users[userEmail].refCode === referralCode) {
                        ...
                        // Referrer gets reward (10 coins only, MR is dynamic)
                        users[userEmail].refs = (users[userEmail].refs || 0) + 1;
                        users[userEmail].pendingReferralEarnings = (users[userEmail].pendingReferralEarnings || 0) + 10;
            ...
            // Only add referral rewards if referral code was used
            if (referralCode && referredBy) {
                newUserData.pendingReferralEarnings = 10;
            } else {
                newUserData.pendingReferralEarnings = 0;
            }
    ...
    try {
        ...
        function sendMiningCompleteNotification() {
            ...
            const notification = new Notification('⛏️ Mining Complete!', {
                body: 'Your mining session has ended. Start mining again!',
        ...
        function initApp() {
            ...
            const adBonus = currentUser.adBoostAmount || 0;
        ...
        function claimLevelReward() {
            ...
            try {
                ...
                // Update user's base rate with level bonus, but ensure MAX 5.0
                const users = JSON.parse(localStorage.getItem('users') || '{}');
                const userEmail = localStorage.getItem('currentUser');
                if (users[userEmail]) {
                    const baseRate = 0.25;
                    ...
                    // Check if current total rate would exceed 5.0
                ...
                // Release lock after a delay to prevent rapid re-claims
                setTimeout(() => {
            ...
            document.getElementById('app').innerHTML = `
                    ...
                    ` : `
                        <div class="timer-wrap">
                            ...
                            <div class="earning-box">
                                <p class="earning-text">Earning: <span class="earning-amount">${(d.rate * 24).toFixed(2)} <span style="font-size: 14px;">ARCASTRA</span></span></p>
                            </div>
                        </div>
                    `}
                </div>
                ...
                ${d.mining ? `
                    ...
                    <div style="margin: 12px 0;">
                        ${(() => {
                            ...
                            const adCooldownKey = 'adCooldown_' + localStorage.getItem('currentUser');
                ...
                <div class="stats">
                    <div class="stat">
                        ...
                        <div class="stat-value">${d.rate.toFixed(2)}${d.rate >= 5.0 ? '<span style="font-size: 9px; color: #00ffff; margin-left: 4px; font-weight: 600;">MAX</span>' : ''}</div>
                    ...
                    ` : `
                        <div class="stat" onclick="openReferralsDirectly()" style="cursor: pointer;">
                            <div class="stat-top">
        ...
        function startMine() {
            ...
            if (users[currentUser.email]) {
                // Remove any ad boost from previous session
                const baseRate = 0.25; // Starting rate
                const levelBonus = (users[currentUser.email].level - 1) * 0.05; // Level rewards
                users[currentUser.email].rate = baseRate + levelBonus;
                                users[currentUser.email].adBoostAmount = 0; // Clear ad boost for new session
localStorage.setItem('users', JSON.stringify(users));
        ...
        function completeMine() {
            ...
            // Update displayed rate after clearing ad boost (referral bonus stays dynamic)
            d.activeRefs = calculateActiveRefs();
            d.rate = (currentUser.rate || 0.25) + (d.activeRefs * 0.05);

d.balance += (d.rate * 24); // Dynamic earning based on rate (rate per hour * 24 hours)
            d.miningDays++;
            ...
            // Send notification
            sendMiningCompleteNotification();
        }
        ...
        function watchAdBoost() {
            ...
            if (currentRate >= 5.0) {
                showToast('❌ Already at maximum rate (5.0 MR)', true);
                isWatchingAd = false;
                return;
            }
            
            // Check cooldown
            const adCooldownKey = 'adCooldown_' + userEmail;
            const adCooldownEnd = parseInt(localStorage.getItem(adCooldownKey) || '0');
            const now = Date.now();
            
            if (now < adCooldownEnd) {
                const remaining = Math.ceil((adCooldownEnd - now) / 1000);
                showToast(`⏳ Please wait ${remaining} seconds`, true);
                isWatchingAd = false;
                return;
            }
            ...
            setTimeout(() => {
                // Check again if rate is still under 5.0
                const users = JSON.parse(localStorage.getItem('users') || '{}');
                const user = users[userEmail];
                
                if (!user) return;
                
                // Calculate current total rate (base + referral + existing ad boost)
                const baseRate = user.rate || 0.25; // permanent (level) base rate
                const activeRefs = calculateActiveRefs();
                const refBonus = activeRefs * 0.05;
                const currentAdBoost = user.adBoostAmount || 0;
                const currentTotalRate = baseRate + refBonus + currentAdBoost;

                // Add boost if possible (session-only)
                const remainingToMax = 5.0 - currentTotalRate;
                const addedBoost = Math.min(0.25, Math.max(0, remainingToMax));
                ...
                user.adBoostAmount = currentAdBoost + addedBoost;
                showToast(`✓ Ad watched! +${addedBoost.toFixed(2)} MR boost added for this session`);
                ...
                const cooldownEnd = Date.now() + (15 * 1000);
                localStorage.setItem(adCooldownKey, cooldownEnd.toString());
        ...
        function shareProfile() {
            const shareText = `Check out my Arc Astra profile!\n\n👤 ${currentUser.username}\n⭐ Level ${d.level}\n💰 ${d.balance.toFixed(2)} ARCASTRA\n🔗 Referral Code: ${currentUser.referralCode}`;
            
            if (navigator.share) {
                navigator.share({
                    title: 'Arc Astra Profile',
                    text: shareText
                }).catch(() => {
        ...
        function viewRoadmap() {
            // TODO: Add external URL redirect here
            // Example: window.open('https://arcastra.com/roadmap', '_blank');
        }
        
        function viewWhitepaper() {
            // TODO: Add whitepaper URL redirect here
            // Example: window.open('https://arcastra.com/whitepaper.pdf', '_blank');
        }
        ...
        function openPage(page) {
            ...
            switch(page) {
                ...
                case 'leaderboard':
                    ...
                    pageContent.innerHTML = getLeaderboardPageContent();
                    break;
                case 'referrals':
                    pageTitle.textContent = 'My Referrals';
                    pageContent.innerHTML = getReferralsPageContent();
                    break;
                case 'menu2':
        ...
        function getLeaderboardPageContent() {
            // Check cache first (updates at 00:00 UTC daily)
            const cacheKey = 'leaderboard_cache';
            const cacheTimeKey = 'leaderboard_cache_time';
            ...
            // Cache is valid if it was created today (UTC)
            const cacheValid = cacheTime && parseInt(cacheTime) >= todayUTCTimestamp;
            
            let leaderboardData = [];
            
            if (cacheValid && cacheData) {
                // Use cached data
                leaderboardData = JSON.parse(cacheData);
            } else {
                // Generate new leaderboard from localStorage
                const users = JSON.parse(localStorage.getItem('users') || '{}');
                leaderboardData = Object.entries(users)
                    ...
                    .sort((a, b) => {
                        ...
                        // If levels are equal, sort by miningDays (descending)
                        return b.miningDays - a.miningDays;
                    })
                    .slice(0, 100); // Top 100
                
                // Save to cache with today's UTC date
                localStorage.setItem(cacheKey, JSON.stringify(leaderboardData));
                localStorage.setItem(cacheTimeKey, todayUTCTimestamp.toString());
            }
            
            // Find current user's rank
            const currentEmail = localStorage.getItem('currentUser');
            const currentUserRank = leaderboardData.findIndex(u => u.email === currentEmail) + 1;
            const currentUserData = leaderboardData.find(u => u.email === currentEmail);
            ...
            // Current user card (if in top 100)
            if (currentUserRank > 0 && currentUserData) {
                // Calculate current user progress
                const userCurrentReq = levelRequirements[currentUserData.level] || 0;
                const userNextReq = levelRequirements[currentUserData.level + 1];
                let userProgressPercent = 0;
                
                if (currentUserData.level >= 20) {
                    userProgressPercent = 100;
                } else if (userNextReq) {
                    userProgressPercent = Math.min(((currentUserData.miningDays - userCurrentReq) / (userNextReq - userCurrentReq)) * 100, 100);
                }
                
                html += `
                    <div style="background: rgba(255, 215, 0, 0.1); border: 1px solid #fbbf24; border-radius: 10px; padding: 10px; margin-bottom: 6px;">
                            ...
                            <div style="display: flex; align-items: center; gap: 6px;">
                                <div class="level-progress-bar" style="width: 70px; height: 16px; background: rgba(10, 20, 40, 0.85); border-radius: 8px; overflow: hidden; position: relative; border: 1px solid rgba(16, 185, 129, 0.3);">
                                    ...
                                    <div class="level-progress-text" style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); font-size: 8px; font-weight: 700; color: white; text-shadow: 0 1px 2px rgba(0, 0, 0, 0.8);">
                                        ${currentUserData.level >= 20 ? `365/365 Days` : `${currentUserData.miningDays}/${userNextReq || '?'} Days`}
                                    </div>
                                </div>
                                <div style="text-align: right;">
                                    <div style="color: #00ffff; font-weight: 800; font-size: 14px;">Lvl ${currentUserData.level}</div>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            leaderboardData.forEach((user, index) => {
                ...
                // Calculate progress
                const currentReq = levelRequirements[user.level] || 0;
                const nextReq = levelRequirements[user.level + 1];
                ...
                if (user.level >= 20) {
                    ...
                    progressPercent = Math.min(((user.miningDays - currentReq) / (nextReq - currentReq)) * 100, 100);
                    progressText = `${user.miningDays}/${nextReq} Days`;
                } else {
                    progressPercent = 100;
                    progressText = `${user.miningDays} Days`;
                }
                
                html += `
                    <div style="background: rgba(0, 0, 0, 0.3); border: 1px solid rgba(0, 255, 255, 0.2); border-radius: 10px; padding: 10px; ${isCurrentUser ? 'box-shadow: 0 0 10px rgba(139, 92, 246, 0.3);' : ''}">
                        <div style="display: flex; align-items: center; justify-content: space-between;">
                            <div style="display: flex; align-items: center; gap: 10px; flex: 1;">
                                ${isMedal ? 
                                ...
                                <div style="flex: 1; min-width: 0;">
                                    <div style="color: ${isCurrentUser ? '#8b5cf6' : (rank <= 3 ? '#00ffff' : 'white')}; font-weight: 700; font-size: 14px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
                                        ${getCountryFlag(user.country || 'Turkey')} ${user.username}${isCurrentUser ? ' (You)' : ''}
                                    </div>
                                </div>
                            </div>
                            <div style="display: flex; align-items: center; gap: 6px;">
                                <div class="level-progress-bar" style="width: 70px; height: 16px; background: rgba(10, 20, 40, 0.85); border-radius: 8px; overflow: hidden; position: relative; border: 1px solid rgba(16, 185, 129, 0.3);">
                                    ...
                                    <div class="level-progress-text" style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); font-size: 8px; font-weight: 700; color: white; text-shadow: 0 1px 2px rgba(0, 0, 0, 0.8);">
                                        ${user.level >= 20 ? `365/365 Days` : `${user.miningDays}/${nextReq || '?'} Days`}
                                    </div>
                                </div>
                                <div style="text-align: right;">
                                    <div style="color: #00ffff; font-weight: 800; font-size: 14px;">Lvl ${user.level}</div>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        function getReferralsPageContent() {
            const users = JSON.parse(localStorage.getItem('users') || '{}');
            const currentEmail = localStorage.getItem('currentUser');
            ...
            for (let email in users) {
                ...
                if (user.referredBy === currentUser.refCode) {
                    const miningEndTime = localStorage.getItem('miningEndTime_' + email);
                    const isMining = miningEndTime && now < parseInt(miningEndTime);
                    ...
                    if (isMining) {
                        activeRefs.push(refData);
                    } else {
                        inactiveRefs.push(refData);
                    }
                }
            }
        ...
        function claimReferralEarnings() {
            ...
            if (!users[userEmail] || !users[userEmail].pendingReferralEarnings || users[userEmail].pendingReferralEarnings === 0) {
            ...
            try {
                const earnings = users[userEmail].pendingReferralEarnings;
                const mrReward = users[userEmail].pendingReferralMR || 0;
                
                // Add coins to balance ONLY (not MR)
                users[userEmail].balance = (users[userEmail].balance || 0) + earnings;
                
                // DO NOT add MR to rate - it will be dynamic based on active refs
                ...
                // Reset pending earnings and MR
                users[userEmail].pendingReferralEarnings = 0;
                users[userEmail].pendingReferralMR = 0;
                
                // Save to localStorage
                localStorage.setItem('users', JSON.stringify(users));
                
                // Update current user and d object
                currentUser = users[userEmail];
                d.balance = currentUser.balance;
                d.pendingReferralEarnings = 0;
                d.pendingReferralMR = 0;
                
                // Show success message
                showToast(`✓ Claimed ${earnings} Coins from referrals!`);
        ...
        function savePersonalInfoPage() {
            ...
            d.fullName = document.getElementById('inputFullNamePage').value.trim();
            d.birthDate = document.getElementById('inputBirthDatePage').value.trim();
            d.country = document.getElementById('inputCountryPage').value.trim();
            
            // Validate full name: English letters and spaces only
            const fullNameRegex = /^[a-zA-Z\s]+$/;
            if (!fullNameRegex.test(d.fullName)) {
                showToast('❌ Full Name: Only English letters allowed');
                return;
            }
            if (d.fullName.length < 3) {
            ...
            if (d.birthDate) {
                const dateRegex = /^(\d{2})\/(\d{2})\/(\d{4})$/;
                const match = d.birthDate.match(dateRegex);
                ...
                if (year < 1923 || year > 2007) {
                    showToast('❌ Year: 1923-2007');
                    return;
                }
                
                if (month < 1 || month > 12) {
                    showToast('❌ Month: 1-12');
                    return;
                }
            ...
            // Handle email change in users object
            const users = JSON.parse(localStorage.getItem('users') || '{}');
            const oldEmail = currentUser.email;
            
            if (newEmail !== oldEmail) {
                // Create new entry with new email
                users[newEmail] = {
                    ...users[oldEmail],
                    email: newEmail,
                    fullName: d.fullName,
            ...
            // Refresh profile if email changed
            if (newEmail !== oldEmail) {
                setTimeout(() => {
                    openPage('profile');
                }, 1000);
        ...
        // Splash screen - 2.5 saniye sonra kaybol
